        ...
        if file_name is None:
            ...
            for obj in response.get('Contents', []):
                ...
                if source_key.endswith(file_name):
                    ...
                    logger.info(f"Skipped file: {source_key}  at {bucket_name} bucket as it doesn't match the filename criteria")

        return f"Data Moved successfully from {source_prefix} to {destination_prefix}  at {bucket_name} bucket"
    except Exception as e:
        logger.error(f"Error moving file : {str(e)}")
        traceback_message = traceback.format_exc()
        print(traceback_message)
